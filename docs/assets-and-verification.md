...
| --- | --- | --- |
| `dev-assets/environment/bg_room.exr` | Authored source HDR room/window image for day environment generation and verification. Kept outside `src/` so Vite never treats it as a runtime asset. | Authored development/build input. |
| `src/assets/bg_room_studio.rgba16f` | Exact studio-shaped RGBA16F day environment consumed at runtime. | Generated by `npm run build:environment`. |
| `src/graphics/scene/studio-environment.generated.ts` | Exact measured day light direction, color, source spread, window fraction, and irradiance. | Generated by `npm run build:environment`. |
| `dev-assets/environment/night.exr` | Authored night HDR source used only for environment generation and verification. Kept outside `src/` so Vite does not emit it. | Authored development/build input. |
| `src/assets/night.rgba16f` | Exact unshaped RGBA16F night environment consumed at runtime. | Generated by `npm run build:environment`. |
| `src/graphics/scene/night-environment.generated.ts` | Exact measured night light direction, color, source spread, window fraction, and irradiance. | Generated by `npm run build:environment`. |
| `src/assets/wood_texture/wood_base.jpg` | 8192² desktop table albedo. | Authored runtime asset. |
| `src/assets/wood_texture/wood_base_4k.jpg` | 4096² table albedo exclusively preloaded and selected by the startup-locked mobile path. | Authored runtime asset. |
...
| `npm run build` | Ensure the day/night prebaked environment outputs exist (generate them only if any are missing), then type-check and create the Vite production bundle. |
| `npm run lint` | Run ESLint over `src`, `scripts`, and the config. |
| `npm run typecheck` | Run TypeScript with `noEmit`. |
| `npm run build:model` | Regenerate the packed model assets. |
| `npm run build:environment` | Regenerate both exact RGBA16F runtime environments and their measured-light metadata from the authored EXRs in `dev-assets/environment/`. |
...
| `npm run test:lighting` | Require byte-for-byte and generated-metadata parity for both day and night prebakes, then verify lighting/shadow invalidation. |
| `npm run test:caustics` | Verify the universal `receiveCaustics` opt-in, material deduplication, emissive preservation, lighting updates, and unchanged-frame receiver-buffer reuse. |
